...
@section('content')
<div class="container">
    <h1>تعديل أمر الإنتاج: {{ $productionOrder->order_no }}</h1>
    ...
    <form action="{{ route('production_orders.update', $productionOrder->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('order_no') is-invalid @enderror" 
                   id="order_no" name="order_no" value="{{ old('order_no', $productionOrder->order_no) }}" required>
            @error('order_no')
            ...
            <select class="form-select @error('item_id') is-invalid @enderror" id="item_id" name="item_id" required>
                ...
                @foreach ($finishedGoods as $product)
                    <option value="{{ $product->id }}" 
                        {{ old('item_id', $productionOrder->item_id) == $product->id ? 'selected' : '' }}>
            ...
            <select class="form-select" id="bill_of_material_id" name="bill_of_material_id">
                ...
                @foreach ($boms as $bom)
                    <option value="{{ $bom->id }}" 
                            ...
                            {{ old('bill_of_material_id', $productionOrder->bill_of_material_id) == $bom->id ? 'selected' : '' }}>
                        {{ $bom->finishedGood ? $bom->finishedGood->name : 'غير محدد' }} 
                        (يحتاج: {{ $bom->quantity }} {{ $bom->rawMaterial ? $bom->rawMaterial->unit_of_measure : 'وحدة' }})
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control @error('quantity') is-invalid @enderror" 
                   id="quantity" name="quantity" value="{{ old('quantity', $productionOrder->quantity) }}" required>
            @error('quantity')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                   ...
                   value="{{ old('start_date', $productionOrder->start_date ? \Carbon\Carbon::parse($productionOrder->start_date)->format('Y-m-d') : date('Y-m-d')) }}" required>
            @error('start_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('end_date') is-invalid @enderror" 
                   ...
                   value="{{ old('end_date', $productionOrder->end_date ? \Carbon\Carbon::parse($productionOrder->end_date)->format('Y-m-d') : '') }}">
            @error('end_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
            ...
            <select class="form-select" id="work_center_id" name="work_center_id">
                ...
                @foreach ($workCenters as $workCenter)
                    <option value="{{ $workCenter->id }}" 
                        {{ old('work_center_id', $productionOrder->work_center_id) == $workCenter->id ? 'selected' : '' }}>
                        {{ $workCenter->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            <button type="submit" class="btn btn-primary">تحديث أمر الإنتاج</button>
