<div class="row mb-3 align-items-end">
    <div class="col-md-4">
        <label for="item_id_{{ $index }}" class="form-label">Product</label>
        <select class="form-select" id="item_id_{{ $index }}" name="items[{{ $index }}][item_id]" required onchange="updateItemPrice(this, {{ $index }})">
            ...
            @foreach ($products as $product)
                <option value="{{ $product->id }}" data-price="{{ $product->selling_price }}" {{ (isset($item['item_id']) && $item['item_id'] == $product->id) ? 'selected' : '' }}>{{ $product->name }} ({{ $product->code }})</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-2">
    ...
    <div class="col-md-2">
        <label for="unit_price_{{ $index }}" class="form-label">Unit Price</label>
    ...
    <div class="col-md-2">
        <label for="line_total_{{ $index }}" class="form-label">Line Total</label>
