...
@section('content')
<div class="container">
    <h1>إنشاء قائمة مواد جديدة (BOM)</h1>
    ...
    <form action="{{ route('boms.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('finished_good_id') is-invalid @enderror" id="finished_good_id" name="finished_good_id" required>
                ...
                @foreach ($finishedGoods as $product)
                    <option value="{{ $product->id }}" {{ old('finished_good_id') == $product->id ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            <button type="submit" class="btn btn-primary">إنشاء قائمة المواد</button>
